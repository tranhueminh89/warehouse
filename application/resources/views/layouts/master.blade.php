...
<div class="wrapper">
        ...
        <!--Settings Check-->
            @if(\App\Role::count()<1)
                <div class="alert alert-error">
                    Please set up Roles to unlock all side bar menu items click here <a
            ...
            @if(\App\ModulePermission::count()<1)
                <div class="alert alert-error">
                    Please set up Roles Permissions to  unlock all menu items <a
                            href="{{action('UserRolesController@index')}}" class="btn btn-flat bg-green ">Create
                        Role</a>
                </div>
            @endif
            @if($companySettings->invoiceNumberingFormat == "")
                <div class="alert alert-info">
                    Please set Invoice Order Format in Settings Module.
                </div>
            @endif
            ...
            @if($companySettings->defaultCurrency == "")
                <div class="alert alert-info">
                    Please set Default Currency in Settings Module.
                </div>
            @endif
            @if($companySettings->defaultLpoTaxAmmount== "")
                <div class="alert alert-info">
                    Please set Default LPO Tax Amount in Settings Module.
                </div>
            @endif
            @if($companySettings->city== "")
                <div class="alert alert-info">
                    Please set City in Settings Module.
                </div>
            @endif
            @yield('content')
        </section>
    ...
    <footer class="main-footer">
        ...
        Copyright &copy;{{date("Y")}} {!!env('COMPANY_NAME')!!} , All Rights Reserved
    </footer>
</div>
</body>
