...
@section('title')
    {!!env('COMPANY_NAME')!!} | @lang('viewsalesorder.View Sales Orders')
@endsection

@section('heading')
    @lang('viewsalesorder.View Sales Orders')
@endsection
    ...
    <div class="text-center">
        <div class="btn-group text-center" role="group" aria-label="...">
            <a href="{{action('InvoiceController@create', array('order'=>$salesOrder->id))}}"
               class="btn btn-flat bg-green"><i class="fa fa-file"></i> @lang('viewsalesorder.Invoice')</a>
            <a href="{{action('SalesOrderController@edit', $salesOrder->id)}}" class="btn btn-flat bg-blue"><i
                        class="fa fa-pencil"></i> @lang('viewsalesorder.Edit')</a>
    ...
    <div class="quote">
        <div id="details" class="clearfix">
            ...
            <div id="invoice">
                ...
                <div class="date">@lang('viewsalesorder.Date of Quote'): {{$salesOrder->created_at}}</div>
                <div class="date">@lang('viewsalesorder.All Cost in'): <b>{{$salesOrder->currencyTypeText}}</b></div>
            </div>
        </div>
        <table border="0" cellspacing="0" cellpadding="0">
            ...
            <tr>
                <th class="no">#</th>
                <th class="desc">@lang('viewsalesorder.DESCRIPTION')</th>
                <th class="unit">@lang('viewsalesorder.UNIT PRICE')</th>
                <th class="qty">@lang('viewsalesorder.QUANTITY')</th>
                <th class="tax">@lang('createinvoice.Discount')</th>
                <th class="qty">@lang('viewsalesorder.Tax')</th>
                <th class="total">@lang('viewsalesorder.TOTAL')</th>
            </tr>
            </thead>
            <tbody>
            <?php $count = 1; ?>
            ...
            <tr>
                <td colspan="3"></td>
                <td colspan="3">@lang('viewsalesorder.SUBTOTAL')</td>
                <td>{{number_format($subTotal,2)}}</td>
            </tr>
            <tr>
                <td colspan="3"></td>
                <td colspan="3">@lang('viewsalesorder.TAX')</td>
                <td> {{number_format($salesOrder->items->sum('tax'),2)}}</td>
            </tr>
            <tr>
                <td colspan="3"></td>
                <td colspan="3">@lang('viewsalesorder.GRAND TOTAL')</td>
                <td>{{number_format($salesOrder->items->sum('total')+$salesOrder->items->sum('tax'),2)}}</td>
            </tr>
            </tfoot>

        </table>
        <div id="thanks">@lang('viewsalesorder.Thank you!')</div>
        <div id="notices">
            <div>@lang('viewsalesorder.NOTICE'):</div>
