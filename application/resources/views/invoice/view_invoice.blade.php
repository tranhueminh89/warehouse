...
@section('title')
    {!!env('COMPANY_NAME')!!} | @lang('viewinvoice.View Invoice Details')
@endsection

@section('heading')
   @lang('viewinvoice.View Invoice Details')
@endsection
    ...
    <div class="quote">
        <div id="details" class="clearfix">
            ...
            <div id="invoice">
                ...
                <div class="date">@lang('viewinvoice.Invoice Date'): {{$invoice->invoiceDate}}</div>
                <div class="date">@lang('viewinvoice.Due Date'): {{$invoice->dueDate}}</div>
                <div class="date">@lang('ciewinvoice.All Cost in'): <b>{{$invoice->currencyTypeText}}</b></div>
            </div>
        </div>
        <table border="0" cellspacing="0" cellpadding="0">
            ...
            <tr>
                <th class="no">#</th>
                <th class="desc">@lang('viewinvoice.DESCRIPTION')</th>
                <th class="unit">@lang('viewinvoice.UNIT PRICE')</th>
                <th class="qty">@lang('viewinvoice.QUANTITY')</th>
                <th class="tax">@lang('createinvoice.Discount')</th>
                <th class="qty">@lang('viewinvoice.Tax')</th>
                <th class="total">@lang('viewinvoice.TOTAL')</th>
            </tr>
            </thead>
            <tbody>
            <?php $count = 1; ?>
            ...
            @foreach($items as $item)
                <tr>
                    ...
                    <td class="total">{{number_format((($item->convertedPrice * $item->qty)+$item->tax),2)}}</td>
                    <?php $count++; ?>
                    <?php $subTotal += $item->convertedPrice * $item->qty;?>
                </tr>
            @endforeach
            </tbody>
            <tfoot>
            <tr>
                <td colspan="3"></td>
                <td colspan="3">@lang('viewinvoice.SUBTOTAL')</td>
                <td>{{number_format($subTotal,2)}}</td>
            </tr>
            <tr>
                <td colspan="3"></td>
                <td colspan="3">@lang('viewinvoice.TAX')</td>
                <td> {{number_format($invoice->items->sum('tax'),2)}}</td>
            </tr>
            <tr>
                <td colspan="3"></td>
                <td colspan="3">@lang('viewinvoice.GRAND TOTAL')</td>
                <td>{{number_format($invoice->items->sum('total')+$invoice->items->sum('tax'),2)}}</td>
            </tr>
            </tfoot>

        </table>
        <div id="thanks">@lang('viewinvoice.Thank you!')</div>
        <div id="notices">
            <div>@lang('viewinvoice.NOTICE'):</div>
